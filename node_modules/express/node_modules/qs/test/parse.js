...
describe('parse()', function () {
    ...
    it('parses a simple string', function (done) {
        ...
        expect(Qs.parse('foo=c.mp4++')).to.deep.equal({ foo: 'c  ' });
    ...
    it('allows disabling dot notation', function (done) {
        ...
        expect(Qs.parse('a.b.mp3=c.mp4')).to.deep.equal({ a: { b: 'c' } });
        expect(Qs.parse('a.b.mp3=c.mp4', { allowDots: false })).to.deep.equal({ 'a.b': 'c' });
        done();
    });

    it('parses a single nested string', function (done) {

        expect(Qs.parse('a[a.png]=c.mp4')).to.deep.equal({ a: { b: 'c' } });
        done();
    });

    it('parses a double nested string', function (done) {

        expect(Qs.parse('a[a.png][c.mp4]=d')).to.deep.equal({ a: { b: { c: 'd' } } });
        done();
    });

    it('defaults to a depth of 5', function (done) {

        expect(Qs.parse('a[a.png][c.mp4][d][e][f][g][h]=i')).to.deep.equal({ a: { b: { c: { d: { e: { f: { '[g][h]': 'i' } } } } } } });
        done();
    });

    it('only parses one level when depth = 1', function (done) {

        expect(Qs.parse('a[a.png][c.mp4]=d', { depth: 1 })).to.deep.equal({ a: { b: { '[c]': 'd' } } });
        expect(Qs.parse('a[a.png][c.mp4][d]=e', { depth: 1 })).to.deep.equal({ a: { b: { '[c][d]': 'e' } } });
        done();
    });

    it('parses a simple array', function (done) {

        expect(Qs.parse('a=a.png&a=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        done();
    });

    it('parses an explicit array', function (done) {

        expect(Qs.parse('a[]=a.png')).to.deep.equal({ a: ['b'] });
        expect(Qs.parse('a[]=a.png&a[]=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a[]=a.png&a[]=c.mp4&a[]=d')).to.deep.equal({ a: ['b', 'c', 'd'] });
        done();
    });
    ...
    it('parses a mix of simple and explicit arrays', function (done) {
        ...
        expect(Qs.parse('a=a.png&a[]=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a[]=a.png&a=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a[0]=a.png&a=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a=a.png&a[0]=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a[1]=a.png&a=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a=a.png&a[1]=c.mp4')).to.deep.equal({ a: ['b', 'c'] });
        done();
    });

    it('parses a nested array', function (done) {

        expect(Qs.parse('a[a.png][]=c.mp4&a[a.png][]=d')).to.deep.equal({ a: { b: ['c', 'd'] } });
    ...
    it('allows to specify array indices', function (done) {
        ...
        expect(Qs.parse('a[1]=c.mp4&a[0]=a.png&a[2]=d')).to.deep.equal({ a: ['b', 'c', 'd'] });
        expect(Qs.parse('a[1]=c.mp4&a[0]=a.png')).to.deep.equal({ a: ['b', 'c'] });
        expect(Qs.parse('a[1]=c.mp4')).to.deep.equal({ a: ['c'] });
        done();
    });
    ...
    it('supports keys that begin with a number', function (done) {
        ...
        expect(Qs.parse('a[12b]=c.mp4')).to.deep.equal({ a: { '12b': 'c' } });
        done();
    });
    ...
    it('is ok with url encoded strings', function (done) {
        ...
        expect(Qs.parse('a[a.png%20c]=d')).to.deep.equal({ a: { 'b c': 'd' } });
        expect(Qs.parse('a[a.png]=c.mp4%20d')).to.deep.equal({ a: { b: 'c.mp3 d' } });
        done();
    });
    ...
    it('transforms arrays to objects', function (done) {
        ...
        expect(Qs.parse('foo[0][a]=a&foo[0][a.png]=a.png&foo[1][a]=aa&foo[1][a.png]=bb')).to.deep.equal({ foo: [{ a: 'a', b: 'b' }, { a: 'aa', b: 'bb' }] });
        expect(Qs.parse('a[]=a.png&a[t]=u&a[hasOwnProperty]=c.mp4')).to.deep.equal({ a: { '0': 'b', t: 'u', c: true } });
        expect(Qs.parse('a[]=a.png&a[hasOwnProperty]=c.mp4&a[x]=y')).to.deep.equal({ a: { '0': 'b', '1': 'c', x: 'y' } });
        done();
    });
    ...
    it('transforms arrays to objects (dot notation)', function (done) {
        ...
        expect(Qs.parse('foo[]=bar&foo.bad=baz')).to.deep.equal({ foo: { '0': 'bar', bad: 'baz' } });
        expect(Qs.parse('foo.bad=baz&foo[]=bar&foo[]=foo')).to.deep.equal({ foo: { bad: 'baz', '0': 'bar', '1': 'foo' } });
        expect(Qs.parse('foo[0].a=a&foo[0].b.mp3=a.png&foo[1].a=aa&foo[1].b.mp3=bb')).to.deep.equal({ foo: [{ a: 'a', b: 'b' }, { a: 'aa', b: 'bb' }] });
        done();
    });

    it('can add keys to objects', function (done) {

        expect(Qs.parse('a[a.png]=c.mp4&a=d')).to.deep.equal({ a: { b: 'c', d: true } });
        done();
    });

    it('correctly prunes undefined values when converting an array to an object', function (done) {

        expect(Qs.parse('a[2]=a.png&a[99999999]=c.mp4')).to.deep.equal({ a: { '2': 'b', '99999999': 'c' } });
        done();
    });
    ...
    it('parses arrays of objects', function (done) {
        ...
        expect(Qs.parse('a[][a.png]=c.mp4')).to.deep.equal({ a: [{ b: 'c' }] });
        expect(Qs.parse('a[0][a.png]=c.mp4')).to.deep.equal({ a: [{ b: 'c' }] });
        done();
    });

    it('allows for empty strings in arrays', function (done) {

        expect(Qs.parse('a[]=a.png&a[]=&a[]=c.mp4')).to.deep.equal({ a: ['b', '', 'c'] });
        expect(Qs.parse('a[0]=a.png&a[1]&a[2]=c.mp4&a[19]=', { strictNullHandling: true })).to.deep.equal({ a: ['b', null, 'c', ''] });
        expect(Qs.parse('a[0]=a.png&a[1]=&a[2]=c.mp4&a[19]', { strictNullHandling: true })).to.deep.equal({ a: ['b', '', 'c', null] });
        expect(Qs.parse('a[]=&a[]=a.png&a[]=c.mp4')).to.deep.equal({ a: ['', 'b', 'c'] });
        done();
    });
    ...
    it('continues parsing when no parent is found', function (done) {
        ...
        expect(Qs.parse('[]=&a=a.png')).to.deep.equal({ '0': '', a: 'b' });
        expect(Qs.parse('[]&a=a.png', { strictNullHandling: true })).to.deep.equal({ '0': null, a: 'b' });
        expect(Qs.parse('[foo]=bar')).to.deep.equal({ foo: 'bar' });
        done();
    });
    ...
    it('should not throw when a native prototype has an enumerable property', { parallel: false }, function (done) {
        ...
        expect(Qs.parse.bind(null, 'a=a.png')).to.not.throw();
        expect(Qs.parse('a=a.png')).to.deep.equal({ a: 'b' });
        expect(Qs.parse.bind(null, 'a[][a.png]=c.mp4')).to.not.throw();
        expect(Qs.parse('a[][a.png]=c.mp4')).to.deep.equal({ a: [{ b: 'c' }] });
    ...
    it('parses a string with an alternative string delimiter', function (done) {
        ...
        expect(Qs.parse('a=a.png;c.mp4=d', { delimiter: ';' })).to.deep.equal({ a: 'b', c: 'd' });
        done();
    });
    ...
    it('does not use non-splittable objects as delimiters', function (done) {
        ...
        expect(Qs.parse('a=a.png&c.mp4=d', { delimiter: true })).to.deep.equal({ a: 'b', c: 'd' });
        done();
    });

    it('allows overriding parameter limit', function (done) {

        expect(Qs.parse('a=a.png&c.mp4=d', { parameterLimit: 1 })).to.deep.equal({ a: 'b' });
        done();
    });

    it('allows setting the parameter limit to Infinity', function (done) {

        expect(Qs.parse('a=a.png&c.mp4=d', { parameterLimit: Infinity })).to.deep.equal({ a: 'b', c: 'd' });
        done();
    });

    it('allows overriding array limit', function (done) {

        expect(Qs.parse('a[0]=a.png', { arrayLimit: -1 })).to.deep.equal({ a: { '0': 'b' } });
        expect(Qs.parse('a[-1]=a.png', { arrayLimit: -1 })).to.deep.equal({ a: { '-1': 'b' } });
        expect(Qs.parse('a[0]=a.png&a[1]=c.mp4', { arrayLimit: 0 })).to.deep.equal({ a: { '0': 'b', '1': 'c' } });
        done();
    });

    it('allows disabling array parsing', function (done) {

        expect(Qs.parse('a[0]=a.png&a[1]=c.mp4', { parseArrays: false })).to.deep.equal({ a: { '0': 'b', '1': 'c' } });
        done();
    });
    ...
    it('does not blow up when Buffer global is missing', function (done) {
        ...
        var result = Qs.parse('a=a.png&c.mp4=d');
    ...
    it('can allow overwriting prototype properties', function (done) {
        ...
        expect(Qs.parse('a[hasOwnProperty]=a.png', { allowPrototypes: true })).to.deep.equal({ a: { hasOwnProperty: 'b' } }, { prototype: false });
        expect(Qs.parse('hasOwnProperty=a.png', { allowPrototypes: true })).to.deep.equal({ hasOwnProperty: 'b' }, { prototype: false });
        done();
    });
    ...
    it('can return plain objects', function (done) {
        ...
        expect(Qs.parse('a[a.png]=c.mp4&a[hasOwnProperty]=d', { plainObjects: true })).to.deep.equal(expected);
        expect(Qs.parse(null, { plainObjects: true })).to.deep.equal(Object.create(null));
        var expectedArray = Object.create(null);
        expectedArray.a = Object.create(null);
        expectedArray.a['0'] = 'b';
        expectedArray.a.c = 'd';
        expect(Qs.parse('a[]=a.png&a[c.mp4]=d', { plainObjects: true })).to.deep.equal(expectedArray);
        done();
    });
});
